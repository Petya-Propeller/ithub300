...
### Игровые ограничения
- Переход между уровнями зума занимает 0.5-3 секунды, в зависимости от системы игрока
...
## Архитектура системы зума

```mermaid
graph LR
    A[Стратегический] --> B[Оперативный]
    B --> C[Тактический]
    C --> B
    B --> A
```
...
sequenceDiagram
    ...
    participant ИИ as Интерфейс
    participant Г as Игровая логика

    И->>К: Изменение зума
    К->>С: Новое значение zoom
    С->>С: Определить уровень
    С->>ИИ: Обновить интерфейс
    ИИ->>Г: Доступные команды
    Г->>ИИ: Подтверждение
    ИИ->>И: Новый интерфейс
```
